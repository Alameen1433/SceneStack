...
const addMediaType = (results, type) => results.map((item) => ({ ...item, media_type: type }));
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const details = await fetchFromTMDB(`${type}/${id}?append_to_response=videos,credits,images`);
