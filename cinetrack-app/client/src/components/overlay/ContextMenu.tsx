...
const useContextMenuActions = (item: ContextMenuItem, onClose: () => void, rect: DOMRect) => {
  const { removeFromWatchlist, toggleMovieWatched, toggleWatchlistFromSearchResult } =
    useWatchlistStore();
...
const DesktopMenu = ({
  ...
  const { viewDetails, addToList, toggleWatched, remove } = useContextMenuActions(
    item,
    onClose,
    rect
  );
  ...
  const btnBase =
    "flex items-center gap-3 px-3 py-2.5 text-sm rounded-xl transition-colors text-left";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-0.5">
        <button
          onClick={viewDetails}
          className={`${btnBase} text-brand-text-light hover:bg-brand-primary/10`}
        >
        ...
        {isMovie && isInWatchlist && (
          ...
          >
            {isPending ? (
              ...
              <FiEye className="text-brand-secondary" />
            ) : (
        ...
        <div className="bg-brand-surface/95 backdrop-blur-3xl rounded-3xl overflow-hidden border border-brand-primary/20 shadow-2xl">
          <div className="p-4 flex gap-4 items-center border-b border-brand-primary/10">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-brand-text-dim capitalize">
          ...
          <div className="p-2 flex flex-col gap-2">
            <div className="bg-brand-bg/50 rounded-2xl overflow-hidden">
              <button
                onClick={viewDetails}
                className={`${btnBase} text-brand-text-light active:bg-brand-primary/10 border-b border-brand-primary/5`}
              >
                <span className="flex items-center gap-3">
                  <FiInfo className="text-xl text-brand-primary" /> View Details
                </span>
              </button>
              ...
              {!isInWatchlist && (
                ...
                >
                  <span className="flex items-center gap-3">
                    {isPending ? <Spinner /> : <FiPlus className="text-xl text-brand-primary" />}{" "}
                    Add to List
                  </span>
                </button>
              )}
                ...
                >
                  <span className="flex items-center gap-3">
                    {isPending ? (
                      ...
                      <FiEye className="text-xl text-brand-secondary" />
                    ) : (
                      <FiCheck className="text-xl text-brand-primary" />
                    )}
