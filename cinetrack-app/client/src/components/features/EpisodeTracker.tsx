...
export const EpisodeTracker: React.FC<{
  ...
  onToggleEpisode: (tvId: number, seasonNumber: number, episodeNumber: number) => void;
  onToggleSeasonWatched: (tvId: number, seasonNumber: number, allEpisodeNumbers: number[]) => void;
}> = ({ tvShow, getSeasonDetails, watchedEpisodes, onToggleEpisode, onToggleSeasonWatched }) => {
  ...
  }, [tvShow.id, tvShow.seasons, fetchSeasonData]);
  ...
  return (
    <div className="relative flex flex-col h-full">
      {showConfetti &&
        createPortal(<Confetti onComplete={() => setShowConfetti(false)} />, document.body)}
      ...
      {!isLoading && seasonData && (
        <ul className="space-y-3 overflow-y-auto flex-grow pr-2">
          {seasonData.episodes.map((episode) => {
            ...
            const needsTruncation = episode.overview && episode.overview.length > 150;

            const isUnaired = episode.air_date ? new Date(episode.air_date) > new Date() : true;
