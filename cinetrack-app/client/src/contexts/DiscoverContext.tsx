...
import { getTrendingMedia, getPopularMovies, getPopularTVShows } from "../services/tmdbService";
...
export const DiscoverProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchDiscoverData = async () => {
      ...
      setError(null);
      try {
        const [trendingResults, popularMoviesResults, popularTVResults] = await Promise.all([
          getTrendingMedia(),
          getPopularMovies(),
          getPopularTVShows(),
        ]);
