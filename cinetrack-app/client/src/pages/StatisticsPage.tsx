  ...
  return (
    <div className="relative flex flex-col items-center justify-center py-10 lg:py-0 lg:h-full">
      ...
      <div className="mt-6 flex items-center gap-3 text-sm text-brand-text-dim/60 font-medium tracking-wider uppercase">
        <span>
          Shows: <span className="text-brand-text-dim">{showsTimeStr}</span>
        </span>
        <span className="w-1 h-1 bg-white/20 rounded-full" />
        <span>
          Movies: <span className="text-brand-text-dim">{moviesTimeStr}</span>
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#121214] border border-white/6 rounded-2xl p-6 flex flex-col justify-between hover:border-white/10 transition-colors duration-300 group">
      <div className="flex items-start gap-4 mb-4">
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center ${iconStyles} shrink-0 group-hover:scale-110 transition-transform duration-300`}
        >
          {icon}
        </div>
        <div>
      ...
      <div>
        <p className="text-brand-text-light text-sm font-medium leading-relaxed">{status}</p>
      </div>
    </div>
  );
};

const CompletionCard: React.FC<{ rate: number; completedCount: number }> = ({
  rate,
  completedCount,
}) => (
  <div className="bg-[#121214] border border-white/6 rounded-2xl p-6 flex items-center gap-6 mt-6 hover:border-white/10 transition-colors duration-300">
    ...
    <div>
      ...
      <p className="text-xs text-white/30 italic">
        Stats get better when you actually watch something.
      </p>
    </div>
  </div>
);
...
export const StatisticsPage: React.FC = memo(() => {
  ...
  const tasteLevel =
    stats.summary.averageRating > 0 ? `${stats.summary.averageRating}/10 Avg` : "Unrated";
  const tasteStatus =
    stats.summary.averageRating > 0
  ...
  return (
    <div
      className={`container mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-opacity duration-300 ${isStale ? "opacity-70" : "opacity-100"}`}
    >
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <StatTile
              ...
              subtitle={
                watchedItems.length > 0 ? "You finish what you start." : "No completed titles yet."
              }
          ...
          <div className="flex flex-wrap gap-2">
            {stats.summary.topGenres.map((genre) => (
              ...
              >
                ...
                <span className="text-xs text-brand-text-dim bg-black/40 rounded-full px-2 py-0.5 min-w-[20px] text-center">
                  {genre.count}
                </span>
              </div>
            ))}
