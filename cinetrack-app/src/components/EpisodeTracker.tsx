        ...
        <ul className="space-y-3 overflow-y-auto flex-grow pr-2">
          {seasonData.episodes.map((episode) => {
            ...
            return (
              <li
                key={episode.id}
                className="flex items-start gap-4 p-2 rounded-lg hover:bg-white/10 transition-colors cursor-pointer"
                onClick={() =>
                  onToggleEpisode(
                    tvShow.id,
                    selectedSeason,
              ...
              >
                {episode.still_path ? (
                  <img
                    src={`${TMDB_IMAGE_BASE_URL.replace("w500", "w300")}${
                      episode.still_path
                    }`}
                    alt={`Still from ${episode.name}`}
                    className="w-32 h-20 object-cover rounded-md flex-shrink-0 bg-brand-surface"
                    loading="lazy"
                  />
                ) : (
                  <EpisodePlaceholder />
                ...
                <div className="flex-grow">
                  <h4 className="font-semibold text-sm text-brand-text-light">
                    {episode.episode_number}. {episode.name}
                  </h4>
                  <p className="text-xs text-brand-text-dim mt-1">{airDate}</p>
                  {episode.overview && (
                    <div>
                      <p
                        className={`text-xs text-brand-text-dim mt-2 leading-snug transition-all ${
                      ...
                      {needsTruncation && (
                        <button
                          ...
                          className="text-xs font-semibold text-brand-primary hover:underline mt-1 focus:outline-none"
                ...
                {watchedEpisodes[selectedSeason]?.includes(
                  episode.episode_number
                ) && (
